...
class DORAComplianceAnalyzer:
    ...
    def _calculate_similarity(self, text1: str, text2: str) -> float:
        ...
        try:
            ...
            prompt = [
                {"role": "system", "content": "Compare the semantic similarity of these two texts on a scale of 0 to 1, considering regulatory and technical context:"},
                {"role": "user", "content": f"Text 1: {text1[:1000]}\nText 2: {text2[:1000]}\nProvide only a number between 0 and 1."}
            ]
            
            llm_response = self.llm(prompt, max_new_tokens=10)
            try:
                llm_sim = float(re.search(r"([0-9]*[.])?[0-9]+", llm_response[0]["generated_text"]).group())
            ...
            print(f"Similarity scores - Traditional: {traditional_sim:.3f}, LLM: {llm_sim:.3f}, Combined: {combined_sim:.3f}")
    ...
    def _calculate_traditional_similarity(self, doc1, doc2):
        ...
        def get_weighted_words(doc):
            ...
            for token in doc:
                if not token.is_stop and len(token.text) > 2:
                    weight = 2.0 if token.ent_type_ or token.pos_ in ['NOUN', 'VERB'] else 1.0
        ...
        intersection_score = sum(min(words1.get(w, 0), words2.get(w, 0)) 
                               for w in set(words1) & set(words2))
        union_score = sum(max(words1.get(w, 0), words2.get(w, 0)) 
                         for w in set(words1) | set(words2))
